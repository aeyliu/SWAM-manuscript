...
args[1] = "/net/snowwhite/home/aeyliu/pima/prediXcan/simulations/utmost-comparison/results/pv"
...
calc.models.leaps = function(Y,X,sample.sizes,max.val,iter,outpar)
{
 ...
 for(i in 1:length(models))
 {
  ...
  write.table(y.utmost,paste(args[1],"/pv-",outpar,"/run",iter,"/Y/Y",i,".txt",sep=""),row.names=F,quote=F,sep=" ",col.names=F)
...
sim.pred = function(n.tissue=10,n.tissue.irrelevant=0,n.samples=100,n.snps=100,n.snps.shared=1,n.snps.specific=1,r2=0.1,r2.cross=0.1,mean.effect.size=0.1,sample.sizes=rep(1,50),diag.val = 1,iter,outpar)
{
 ...
 write.table(X.utmost,paste(args[1],"/pv-",outpar,"/run",iter,"/X",".txt",sep=""),row.names=F,quote=F,sep=" ",col.names=F)
 ...
 write.table(X.utmost.test,paste(args[1],"/pv-",outpar,"/run",iter,"/X.test",".txt",sep=""),row.names=F,quote=F,sep=" ",col.names=F)

 for(index in 1:dim(Y.test)[2])
 {
  Y.utmost.test = data.frame(1:dim(Y.test)[1],Y.test[,index])
  write.table(Y.utmost.test,paste(args[1],"/pv-",outpar,"/run",iter,"/Y.test/Y",index,".txt",sep=""),row.names=F,quote=F,sep=" ",col.names=F)
...
run.sim=function(n.times=1000,n.tissue=10,n.tissue.irrelevant=0,n.samples=100,n.snps=100,n.snps.shared=1,n.snps.specific=1,r2=0.1,r2.cross=0.1,mean.effect.size=0.1,sample.sizes=rep(1,50),diag.val=1,outpar)
{
 ...
 for(i in 1:n.times)
 {
  ...
  test = sim.pred(n.tissue=n.tissue,n.tissue.irrelevant=n.tissue.irrelevant,n.samples=n.samples,n.snps=n.snps,n.snps.shared=n.snps.shared,n.snps.specific=n.snps.specific,r2=r2,r2.cross=r2.cross,mean.effect.size=mean.effect.size,sample.sizes=sample.sizes,diag.val = diag.val,iter=i,outpar=1)
  results[i,]=test[[5]]
...
for(i in 1:N)
{
 sim[[i]] = run.sim(n.times=10000,n.tissue=n.tiss,n.tissue.irrelevant=n.tiss.irrelevant,n.samples=200,n.snps=n.snps,n.snps.shared=1,n.snps.specific=1,r2=r2[i],r2.cross=r2.c,mean.effect.size=0.1,sample.sizes=ss.in,diag.val=dv,outpar=1)
}
...
results.list.sim = matrix(nrow=length(pv.thresholds),ncol=6)

for(i in 1:dim(results.list.sim)[1])
{
 results.list.sim[i,] = compile.results(sim,cor.threshold=cor.thresholds[i])[[1]]
}
