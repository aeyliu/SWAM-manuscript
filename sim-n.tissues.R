...
args[1] = "/net/snowwhite/home/aeyliu/pima/prediXcan/simulations/utmost-comparison/results/n-tissues"
...
calc.models.leaps = function(Y,X,sample.sizes,max.val,iter,outpar)
{
 ...
 for(i in 1:length(models))
 {
  ...
  write.table(y.utmost,paste(args[1],"/n-tissues-",outpar,"/run",iter,"/Y/Y",i,".txt",sep=""),row.names=F,quote=F,sep=" ",col.names=F)
...
sim.pred = function(n.tissue=10,n.tissue.irrelevant=0,n.samples=100,n.snps=100,n.snps.shared=1,n.snps.specific=1,r2=0.1,r2.cross=0.1,mean.effect.size=0.1,sample.sizes=rep(1,50),diag.val = 1,iter,outpar)
{
 ...
 write.table(X.utmost,paste(args[1],"/n-tissues-",outpar,"/run",iter,"/X",".txt",sep=""),row.names=F,quote=F,sep=" ",col.names=F)
 ...
 write.table(X.utmost.test,paste(args[1],"/n-tissues-",outpar,"/run",iter,"/X.test",".txt",sep=""),row.names=F,quote=F,sep=" ",col.names=F)

 for(index in 1:dim(Y.test)[2])
 {
  Y.utmost.test = data.frame(1:dim(Y.test)[1],Y.test[,index])
  write.table(Y.utmost.test,paste(args[1],"/n-tissues-",outpar,"/run",iter,"/Y.test/Y",index,".txt",sep=""),row.names=F,quote=F,sep=" ",col.names=F)
...
n.tiss=c(2,4,6,8,10,20,30,40,50)
n.tiss.irrelevant=c(1,2,3,4,5,10,15,20,25)
...
for(i in 1:N)
{
 ss.in = c(0.5,rep(1,n.tiss[i]-1))
 sim[[i]] = run.sim(n.times=9000,n.tissue=n.tiss[i],n.tissue.irrelevant=n.tiss.irrelevant[i],n.samples=200,n.snps=n.snps,n.snps.shared=1,n.snps.specific=1,r2=r2,r2.cross=r2.c,mean.effect.size=0.1,sample.sizes=ss.in,diag.val=dv,outpar=n.tiss[i])
}
...
for(i in 1:N)
{
 write.table(sim[[i]],paste("/net/snowwhite/home/aeyliu/pima/prediXcan/simulations/utmost-comparison/swam-results/n-tissues/n-tissues-",n.tiss[i],".txt",sep=""),quote=F,row.names=F,sep="\t")
}
