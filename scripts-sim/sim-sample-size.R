...
args[1] = "/net/snowwhite/home/aeyliu/pima/prediXcan/simulations/utmost-comparison/results/sample-size"
...
calc.models.leaps = function(Y,X,sample.sizes,max.val,iter)
{
 models = vector(mode="list",length=dim(Y)[2])
 n = dim(X)[1]
 n.targ = sample.sizes[1]*n #sample size of target tissue
 for(i in 1:length(models))
 {
  resample=TRUE
  while(resample)
  {
   ...
   y = Y[ind,i]
   X.temp = X[ind,] 
   rowsum.X = apply(X.temp,2,sum) #check rowsums are all positive
   if(sum(rowsum.X>0)== dim(X.temp)[2])
   {
    resample=FALSE
   }
  }
  ...
  write.table(y.utmost,paste(args[1],"/target-ss-",n.targ,"/run",iter,"/Y/Y",i,".txt",sep=""),row.names=F,quote=F,sep=" ",col.names=F)
...
calc.models = function(Y,X,sample.sizes,iter)
{
 models = vector(mode="list",length=dim(Y)[2])
 n = dim(X)[1]
 for(i in 1:length(models))
...
sim.pred = function(n.tissue=10,n.tissue.irrelevant=0,n.samples=100,n.snps=100,n.snps.shared=1,n.snps.specific=1,r2=0.1,r2.cross=0.1,mean.effect.size=0.1,sample.sizes=rep(1,50),diag.val = 1,iter)
{
 redo.all=TRUE
 while(redo.all)
 {
  ...
  models = calc.models.leaps(Y=Y,X=X,sample.sizes=sample.sizes,max.val=n.snps.shared+n.snps.specific,iter) #iter is the iteration of the simulation
  for(i in 1:length(models))
  {
 ...
 n.targ = dim(X)[1]*sample.sizes[1]
 X.utmost = data.frame(1:dim(X)[1],X)
 write.table(X.utmost,paste(args[1],"/target-ss-",n.targ,"/run",iter,"/X",".txt",sep=""),row.names=F,quote=F,sep=" ",col.names=F)
 ...
 write.table(X.utmost.test,paste(args[1],"/target-ss-",n.targ,"/run",iter,"/X.test",".txt",sep=""),row.names=F,quote=F,sep=" ",col.names=F)

 for(index in 1:dim(Y.test)[2])
 {
  Y.utmost.test = data.frame(1:dim(Y.test)[1],Y.test[,index])
  write.table(Y.utmost.test,paste(args[1],"/target-ss-",n.targ,"/run",iter,"/Y.test/Y",index,".txt",sep=""),row.names=F,quote=F,sep=" ",col.names=F)
 ...
 names.results = c("naive","best.tissue","meta","single")
...
#sample.sizes=rep(1,50)
#diag.val=1

#####################
#run the simulation
run.sim=function(n.times=1000,n.tissue=10,n.tissue.irrelevant=0,n.samples=100,n.snps=100,n.snps.shared=1,n.snps.specific=1,r2=0.1,r2.cross=0.1,mean.effect.size=0.1,sample.sizes=rep(1,50),diag.val=1)
{
 results = matrix(nrow=n.times,ncol=4)
 ...
 for(i in 1:n.times)
 {
  ...
  test = sim.pred(n.tissue=n.tissue,n.tissue.irrelevant=n.tissue.irrelevant,n.samples=n.samples,n.snps=n.snps,n.snps.shared=n.snps.shared,n.snps.specific=n.snps.specific,r2=r2,r2.cross=r2.cross,mean.effect.size=mean.effect.size,sample.sizes=sample.sizes,diag.val = diag.val,iter=i)
  results[i,]=test[[5]]
  target.proportion.weight[i] = test[[6]]
 }
 output = data.frame(results,target.proportion.weight)
 names(output) = c("naive","best.tissue","meta","single","target.weight")
 return(output)
}
...
{
 threshold.results = matrix(nrow=length(results.list),ncol=4)
 mean.results = matrix(nrow=length(results.list),ncol=4)
 for(i in 1:length(results.list))
 {
  threshold.results[i,] = as.numeric(apply(na.omit(results.list[[i]][,1:4]),2,check.threshold,threshold=cor.threshold))
  mean.results[i,] = as.numeric(apply(na.omit(results.list[[i]][,1:4]),2,mean))
 }
...
mean.effect.size=0.1
ss.target = c(50,100,150,200,250,300,350,400,450,500)
dv = 3
N=length(ss.target)
sim.x = ss.target

ss.temp = rep(0.4,n.tiss)

sim = vector(mode="list",length=N)
for(i in 1:N)
{
 ss.in = ss.temp
 ss.in[1] = ss.target[i]/500
 sim[[i]] = run.sim(n.times=9000,n.tissue=n.tiss,n.tissue.irrelevant=n.tiss.irrelevant,n.samples=500,n.snps=n.snps,n.snps.shared=1,n.snps.specific=1,r2=r2,r2.cross=r2.c,mean.effect.size=0.1,sample.sizes=ss.in,diag.val=dv)
}
...
for(i in 1:N)
{
 write.table(sim[[i]],paste("/net/snowwhite/home/aeyliu/pima/prediXcan/simulations/utmost-comparison/swam-results/sample-size/sample-size-",ss.target[i],".txt",sep=""),quote=F,row.names=F,sep="\t")
}
