...
#args = vector(length=9)
#args[1]="/net/snowwhite/home/aeyliu/pima/prediXcan/simulations/utmost-comparison/results/sample-size/target-ss-150/run1/X.txt"
#args[2]="/net/snowwhite/home/aeyliu/pima/prediXcan/simulations/utmost-comparison/info.txt"
#args[3]="/net/snowwhite/home/aeyliu/pima/prediXcan/simulations/utmost-comparison/results/sample-size/target-ss-150/run1/Y/"
#args[4]=5
#args[5]="/net/snowwhite/home/aeyliu/pima/prediXcan/simulations/utmost-comparison/results/sample-size/target-ss-150/run1/"
#args[6]="test"
#args[7]="/net/snowwhite/home/aeyliu/pima/prediXcan/simulations/utmost-comparison/results/sample-size/target-ss-150/run1/X.test.txt"
#args[8]="/net/snowwhite/home/aeyliu/pima/prediXcan/simulations/utmost-comparison/results/sample-size/target-ss-150/run1/Y.test/"
...
if (is.na(args[9])) {
  ...
  if_verbose = as.logical(args[9])
}

X_external_path = normalizePath(args[7])
Y_external_path = normalizePath(args[8])
Y_external_dir = dir(args[8])
...
if(P){
  ...
  Y_external = list()
  for(t in 1:P){
		Y_external[[t]] = read.table(paste0(Y_external_path, '/', Y_external_dir[t]), header=F)
		Y_external[[t]] = as.numeric(Y_external[[t]][,2])
	}
	X_external = read.table(X_external_path, header = F)
	X_external_list = list()
	for(t in 1:P){
		X_external_list[[t]] = as.matrix(X_external[,-1])
	}
  ...
  cat('training a model on entire data with parameters chosen from cv\n')
	...
	external_results = multi_mse(ans$est,X_external_list, Y_external) #use effect sizes on external simulated dataset
	utmost_cor = cor(external_results[[1]][,1],external_results[[1]][,2])
  cat('writing results\n')
  cat(paste(args[6],"utmost_correlation.txt",sep=""))

	write.table(utmost_cor,paste(args[6],"utmost_correlation.txt",sep=""),quote=F,row.names=F,sep="\t",col.names=F)
